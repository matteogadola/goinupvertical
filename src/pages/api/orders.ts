...
const createOrder = async (orderData: Pick<Order, 'payment_method' | 'items'>) => {
  ...
  try {
    ...
    const { rows: orders } = await client.query<Omit<Order, 'items'>>(`
      INSERT INTO orders (date, payment_method)
      VALUES($1, $2) RETURNING *`,
    [dt().utc(), orderData.payment_method])
    const order = orders[0]

    for (let item of orderData.items) {
      await client.query(`
        INSERT INTO order_details (order_id, item_id, description)
        VALUES($1, $2, $3) RETURNING id`,
      [order.id, item.id, item?.description])
      ...
      if (item?.entry) {
        ...
        const { rows: entries } = await client.query<Entry>(`
          ...
          RETURNING *`,
        [ order.id, item.id, item.entry.first_name, item.entry.last_name,
          item.entry.birth_date ?? `${cf.year}-${String(cf.month).padStart(2, '0')}-${String(cf.day).padStart(2, '0')}`,
          item.entry.birth_place ?? cf.birthplace, item.entry.gender ?? cf.gender,
          item.entry.country, item.entry.team, item.entry.email, item.entry.phone_number, item.entry.tin
        ])
